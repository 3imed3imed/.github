...
def test_ambient_bed_is_generated_and_timed(settings, tmp_path):
    from app.rendering import ffmpeg

    out = tmp_path / "bed.wav"
    ffmpeg.generate_ambient_bed(out, duration=4.0)
    assert out.exists()
    dur = ffmpeg.probe_duration(out)
    assert dur is not None and 3.5 <= dur <= 4.5  # matches requested duration


def test_audio_stage_mixes_bed_by_default(settings):
    from app.audio import AudioStage
    from app.models import Scene
    from app.rendering import ffmpeg
    from app.storage import ProjectStore

    store = ProjectStore("t_audio", settings)
    scenes = [
        Scene(scene_id=1, narration="A short narration line for the first scene.", duration=3),
        Scene(scene_id=2, narration="A second short narration line here.", duration=3),
    ]
    result = AudioStage(settings).synthesize(store, scenes)
    assert result.master.exists()
    assert (store.path("audio") / "bed.wav").exists()  # bed generated by default
    assert ffmpeg.probe_duration(result.master) > 0


def test_audio_stage_respects_music_bed_disabled(monkeypatch):
    from app.config import reload_settings

    monkeypatch.setenv("MUSIC_BED_ENABLED", "false")
    s = reload_settings()
    from app.audio import AudioStage
    from app.models import Scene
    from app.storage import ProjectStore

    store = ProjectStore("t_audio_nobed", s)
    result = AudioStage(s).synthesize(store, [Scene(scene_id=1, narration="Only narration, no bed.", duration=3)])
    assert result.master.exists()
    assert not (store.path("audio") / "bed.wav").exists()  # disabled -> no bed
    reload_settings()
